...
wasm: ${API_DIR}source/api.cpp
	emcc -std=c++17 -g -sEXPORT_NAME="'initializeCPPAPI'" -sMODULARIZE=1 -s -sALLOW_MEMORY_GROWTH=1 -sMAXIMUM_MEMORY=4096MB -sINITIAL_MEMORY=1024MB -sTOTAL_STACK=256MB -sTOTAL_MEMORY=2048MB -O3 -sASSERTIONS=1 ${API_MAIN} ${API_SRC} --post-js ${API_DIR}source/api_post.js -o ${WASM_OUT}api.js ${API_INC} -s"EXPORTED_FUNCTIONS=${EXPORTED_FUNCTIONS}" -s"EXPORTED_RUNTIME_METHODS=${EXTRA_EXPORTED_RUNTIME_METHODS}"
clean:
