...
const TObject& TEvent::get(int id){
    ...
    double read_time = time - glm::length(obj->position-vantage)/info_speed ;
...
void TEvent::addEvent(TEvent e){
    e.time = max(e.time, time + timeline->min_spawned_event_delay);
    vec3 vantage = timeline->objects[anchor_id].get(time).position; // TODO cache
    vec3 new_loc = timeline->objects[e.anchor_id].get(e.time) ; //TODO is this sufficient or do we need to iterate to consider movement away?
    double earliest_time = time + glm::length(vantage-new_loc)/timeline->info_speed ;
    e.time =  max(e.time, earliest_time);
...
vector<int> TEvent::getCollisions(){
