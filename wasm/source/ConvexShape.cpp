...
ConvexShape::ConvexShape(std::vector<Polygon>& polygons){
    ...
    for(int k=0;k<polygons.size();k++){
        ...
        for(int j=0;j<poly.p.size();j++){
            ...
            for(int i=0;i<vertex.size();i++){ // TODO could make this much faster
                if(glm::length(poly.p[j]-vertex[i]) < Polygon::EPSILON){
                    index = i;
                    break;
                }
            }
    ...
    radius = 0 ;
    for(int k=0;k<vertex.size();k++){
        float r = glm::length(vertex[k] - dvec3(position));
        if(r > radius){
            radius = r ;
        }
    }
    ...
    for(int k=0;k<face.size();k++){
        dvec3& A = vertex[face[k][0]] ;
        dvec3& B = vertex[face[k][1]] ;
        dvec3& C = vertex[face[k][2]] ;
        dvec3 normal = glm::normalize(glm::cross(B - A, C - A));
        if(glm::dot(normal,A-dvec3(position)) < 0){
            vector<int> new_face ;
...
std::map<std::string,Variant> ConvexShape::serialize() const{
    ...
    serial["v"].makeFillableDoubleArray(vertex.size()*3);
    double* vp = serial["v"].getDoubleArray();
    for(int k=0;k<vertex.size();k++){
        vp[3*k] = vertex[k].x;
        vp[3*k+1] = vertex[k].y;
        vp[3*k+2] = vertex[k].z;
...
void ConvexShape::set(std::map<std::string,Variant>& serialized){
    ...
    vertex = vector<dvec3>();
    double* vp = serialized["v"].getDoubleArray();
...
glm::dvec3 ConvexShape::getCentroid(){
    // Get a point on the inside
    dvec3 inner_point(0,0,0) ;
    ...
    for(const auto& f : face){
        for(int k=1; k < f.size()-1;k++){
            dvec3& a = vertex[f[0]];
            dvec3& b = vertex[f[k]];
            dvec3& c = vertex[f[k+1]];
            dvec3& d = inner_point ;
            double vol = computeTetraVolume(a, b, c, d);
            dvec3 ctr =  computeTetraCentroid(a, b, c, d);
            volume += vol;
            centroid += ctr*vol;
        }
    }
...
glm::dvec3 ConvexShape::centerOnCentroid(){
    dvec3 centroid = getCentroid();
    for(dvec3& v : vertex){
        v-=centroid;
    }
    position = dvec3(0,0,0);
...
float ConvexShape::computeTetraVolume(const glm::dvec3& a, const glm::dvec3& b, const glm::dvec3& c, const glm::dvec3& d){
    return fabs(glm::dot(a-d, glm::cross(b-d,c-d)))/6.0f ;
}

// Returns the center of mass of the given tetrahedron
glm::dvec3 ConvexShape::computeTetraCentroid(const glm::dvec3& a, const glm::dvec3& b, const glm::dvec3& c, const glm::dvec3& d){
    return (a+b+c+d)/4.0;
}

// Returns the inertia tensor of the given tetrahedron about the origin
glm::mat3 ConvexShape::computeTetraInertia(const float mass, const dvec3& A, const dvec3& B, const dvec3& C, const dvec3& D){
